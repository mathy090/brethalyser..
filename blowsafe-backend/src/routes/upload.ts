/**
 * blowsafe-backend/src/routes/upload.ts
    ...
    try {
      ...
      if (uploadError) {
        ...
        if (uploadError.message.includes("Bucket not found")) {
          res.status(500).json({ code: "BUCKET_NOT_FOUND", message: "Supabase 'driver-photos' bucket doesn't exist." });
          return;
        }
        if (uploadError.message.includes("permission") || (uploadError as any).statusCode === 403) {
          res.status(500).json({ code: "PERMISSION_DENIED", message: "Service role key lacks write access to bucket." });
          return;
        }
        res.status(500).json({ code: "UPLOAD_FAILED", message: uploadError.message });
        return;
      }

      // ── 2. Build public URL ───────────────────────────────────────────────
      const photoUrl = `${process.env.SUPABASE_URL}/storage/v1/object/public/driver-photos/${filePath}`;
      ...
      if (driverError) {
        ...
        if ((driverError as any).code === "23505") {
          ...
          res.status(409).json({ code: "DUPLICATE_ENTRY", message: `Driver with this ${field} already exists.` });
          return;
        }
        res.status(500).json({ code: "DATABASE_ERROR", message: "Failed to save driver data." });
        return;
      }
      ...
      if (readingError) {
        ...
        res.status(500).json({ code: "DATABASE_ERROR", message: "Failed to save BAC reading." });
        return;
      }
